    ...
    return (<>
      <Card className="hover:shadow-lg transition-all duration-300 hover:scale-[1.02] rounded-2xl border-2">
        <CardHeader className="flex flex-row items-start justify-between space-y-0 pb-3">
          ...
          <DropdownMenu>
            ...
            <DropdownMenuContent align="end" className="rounded-xl">
              <DropdownMenuItem onClick={handleOpenChat} disabled={document.status !== "completed"} className="rounded-lg">
                <MessageSquare className="h-4 w-4 mr-2"/>
                Abrir Chat
              </DropdownMenuItem>
              <DropdownMenuItem onClick={() => setShowDeleteDialog(true)} className="text-destructive rounded-lg">
        ...
        <CardFooter className="pt-0">
          <Button onClick={handleOpenChat} disabled={document.status !== "completed"} className="w-full rounded-xl" size="sm">
            <MessageSquare className="h-4 w-4 mr-2"/>
      ...
      <Dialog open={showDeleteDialog} onOpenChange={setShowDeleteDialog}>
        <DialogContent className="rounded-2xl">
          ...
          <DialogFooter>
            ...
            <Button variant="destructive" onClick={handleDelete} disabled={deleteDocument.isPending} className="rounded-xl">
