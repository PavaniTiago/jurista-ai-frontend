...
export function SourceViewer({ sources }: SourceViewerProps) {
  ...
  return (
    <div className="space-y-2 overflow-visible relative z-10">
      <Button
        variant="outline"
        size="sm"
        onClick={() => setExpanded(!expanded)}
        className="w-full justify-between rounded-xl hover:bg-primary/5"
      >
      ...
      {expanded && (
        <div className="space-y-3 max-h-[400px] overflow-y-auto pr-2">
          {sources.map((source, index) => (
            <Card
              key={`${source.metadata.pageNumber || 0}-${source.content.substring(0, 50)}-${index}`}
              className="text-sm rounded-xl border-2 overflow-visible"
            >
